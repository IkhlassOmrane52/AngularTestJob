...
export class MapComponent implements OnInit {
  DateChoisie: string;
  object:any;
  options: string[] = ['Aujourdhui', 'Hiér',  'Semaine précédente','Semaine courrante',   'Mois précédent','Mois courant'];
tabToday:any[];
 ...
 nom_user:string;
 id_user:number;
  constructor(@Inject(MAT_DIALOG_DATA) public data:any,public dialogRef: MatDialogRef<MapComponent> ,private dialog: MatDialog,private service:UserService,
 ...
 showDetails(option) {
    ...
    if(option ==='Aujourdhui'){
      ...
      if(data.length!==0){
        ...
        this.toastrService.error('Aucune Localisation à la date donnée');
      }
...
chowDetail(data){
  ...
  this.dialog.open(DistrubtionComponent, dialogConfig);
  ...
  PeriodeShowDetail(data){
    ...
    this.dialog.open(DurationPositionComponent, dialogConfig);
